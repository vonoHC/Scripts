...
def main():
    ...
    parser.add_argument("-to","--timeout",type=int,required=False,help="Numero de hilos. Ej. -to 1;    Por Defecto: '1'",default=1)
    ...
    with ThreadPoolExecutor(max_workers=hilos) as ejecutor:
        if maxPort:
            resultado = ejecutor.map(lambda p: funcion(ip or domain,p,timeout), range(1,maxPort + 1))
        elif port:
             port = [int(p) for p in port.split(",")]
             resultado = ejecutor.map(lambda p: funcion(ip or domain,p,timeout), port)
        elif portRange:
            portRange = [int(p) for p in portRange.split("-")]
            resultado = ejecutor.map(lambda p: funcion(ip or domain,p,timeout), range(portRange[0],portRange[1] + 1))
